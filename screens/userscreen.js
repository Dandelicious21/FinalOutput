...
export default function UserScreen(props) {
  ...
  const searchPlants = async(text) => {
    ...
    await config .get("users/sortIndex/"+id)
    .then((res) => {
      plantObj["sortType"] = res.data.message.sortingType;
      config .post("plants/search/"+id,plantObj)
      .then((res) => {
        if(text === ""){
  ...
  const sortPlants = async() => {
    ...
    await config.post('users/update/'+id,dataObj)
      .then(res => {
         if(res.data.status == "error") console.log(res.data.message);
      })
      .catch(err => {
  ...
  function showSortToast(){
    ToastAndroid.show('Sorting...', ToastAndroid.SHORT);
  }
          ...
          <ScrollView showsVerticalScrollIndicator={false} 
           ...
           {props.loadPlantList && props.loadPlantList.map((data,index) => (
              <TouchableOpacity onPress={() => viewPlant(data)} 
                ...
                <Image source={{uri:'http://192.168.254.102:19000/backend/uploads/'+data["imgLink"]}} 
